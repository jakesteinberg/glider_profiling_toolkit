...
from tools import find_nearest, cart2pol, pol2cart, nanseg_interp, group_consecutives, plot_pro
...
class Glider(object):
    ...
    # Glider Processing Functions

    # Bin-Average Profiles in on a specified grid
    ...
    # this function will separate a set of dive-climb cycles into transects that are each bounded by glider targets. The
    # target associated with each dive-climb cycle is used to separate profiles into transects that are terminated when
    # glider target changes (when it reaches a target and begins to head towards a new one). If transects are repeated,
    # each occupation of the line is treated as its own transect
    def transect_cross_section_1(self, bin_depth, sig0_0, ct_0, sa_0,
                                 lon_0, lat_0, dac_u_0, dac_v_0, profile_tags_0, sigth_levels, include_ends):
        deep_shr_max = 0.1
    ...
    # Cross Section of a Single Glider Transect (Velocity Contoured in Color and Density Surfaces plotted)
    def plot_cross_section(self, bin_depth, ds, v_g, dist, profile_tags, isopycdep, isopycx, sigth_levels, time, levels):
            ...
            ax0.set_title(self.ID + ' ' + self.project + '  ' + np.str(
                t_s.month) + '/' + np.str(t_s.day) + ' - ' + np.str(t_e.month) + '/' + np.str(t_e.day), fontsize=13)
            plt.colorbar(vc, label='[m/s]')
            plt.tight_layout()
            plot_pro(ax0)
            return fig0

    # Conservative Temperature and Absolute Salinity (used to check for instrument drift)
    def plot_t_s(self, ct, sa, time):
        ...
        rr = np.arange(0, 1, 1/np.shape(sa)[1])

        matplotlib.rcParams['figure.figsize'] = (10, 4)
        f, (ax1, ax2) = plt.subplots(1, 2)
        dc = ax1.contour(sa_gr, ct_gr, sig0_gr, levs, colors='#A9A9A9', linewidth=0.2)
        ax1.clabel(dc, inline=1, fontsize=5, fmt='%1.2f', color='k')
        for i in range(self.num_profs):
            ax1.scatter(sa[:, i], ct[:, i], s=2, color=cmap(rr[i]))
        ax1.axis([np.round(np.nanmin(sa), 1) - .2, np.round(np.nanmax(sa), 1) + .2,
                 np.round(np.nanmin(ct), 1) - .5, np.round(np.nanmax(ct), 1) + .5])
        ax1.set_xlabel('Absolute Salinity')
        ax1.set_ylabel('Conservative Temperature')
        t_s = datetime.date.fromordinal(np.int(time[0, 0]))
        t_e = datetime.date.fromordinal(np.int(time[0, -1]))
        ax1.set_title(self.ID + '  ' + self.project + '  ' + np.str(
            t_s.month) + '/' + np.str(t_s.day) + ' - ' + np.str(t_e.month) + '/' + np.str(t_e.day), fontsize=11)
        ax1.grid()

        # zoom in
        for i in range(self.num_profs):
            ax2.scatter(sa[:, i], ct[:, i], s=1, color=cmap(rr[i]))
        ax2.axis([np.round(np.nanmin(sa), 1) - .05, np.round(np.nanmin(sa), 1) + 0.12,
                 np.round(np.nanmin(ct), 1) - .1, np.round(np.nanmin(ct), 1) + 2])
        ax2.set_xlabel('Absolute Salinity')
        ax2.set_ylabel('Conservative Temperature')
        ax2.set_title('Deep CT, SA')

        plot_pro(ax2)

    def plot_v(self, bin_depth, v_g, time, profile_tags, u_levels):
        t_s = datetime.date.fromordinal(np.int(time[0, 0]))
        t_e = datetime.date.fromordinal(np.int(time[0, -1]))
        cmap = plt.cm.get_cmap("YlGnBu_r")
        rr = np.arange(0, 1, 1/np.shape(v_g)[1])
        matplotlib.rcParams['figure.figsize'] = (5.5, 6)
        f, ax = plt.subplots()
        for i in range(np.shape(v_g)[1]):
            ax.plot(v_g[:, i], bin_depth, color=cmap(rr[i]))
        ax.set_title(self.ID + '  ' + self.project + '  ' + np.str(
            t_s.month) + '/' + np.str(t_s.day) + ' - ' + np.str(t_e.month) + '/' + np.str(t_e.day), fontsize=10)
        ax.text(0.1, 0.15, 'dive-cycles: ' + str(np.nanmin(profile_tags)) + '-' + str(np.nanmax(profile_tags)),
                fontsize=10)
        ax.set_xlim([np.nanmin(u_levels), np.nanmax(u_levels)])
        ax.set_ylim([-200, np.nanmax(bin_depth) + 100])
        ax.set_ylabel('Depth [m]')
        ax.set_xlabel('Cross-Track Velocity [m/s]')
        ax.invert_yaxis()
        plot_pro(ax)

    # Plan View of Glider Dives for desired mission
    def plot_plan_view(self, lon, lat, mwe_lon, mwe_lat, dac_u, dac_v, ref_lat, profile_tags, time, limits, path):
        ...
        matplotlib.rcParams['figure.figsize'] = (6.5, 4)
        f, ax = plt.subplots()
        bc = ax.contourf(bath_lon, bath_lat, bath_z, levels, cmap='Blues_r', extend='both', zorder=0)
        matplotlib.rcParams['contour.negative_linestyle'] = 'solid'
        bcl = ax.contour(bath_lon, bath_lat, bath_z, [-5000, -4500, -4000, -3500], colors='k', zorder=0)
        ax.scatter(lon, lat, s=2, color='#FF8C00', label='Dive Path')
        ...
        qa = ax.quiver(mwe_lon, mwe_lat, dac_u, dac_v, color='r',
                  scale=1.5, headwidth=2, headlength=2, width=.003)
        ax.quiverkey(qa, .2, .2, 0.1, '0.1 m/s', color='r')
        ax.clabel(bcl, inline_spacing=-3, fmt='%1.0f', colors='k')
        w = 1 / np.cos(np.deg2rad(ref_lat))
        ax.axis(limits)
        ax.set_aspect(w)
        divider = make_axes_locatable(ax)
        cax = divider.append_axes("right", size="5%", pad=0.05)
        f.colorbar(bc, cax=cax, label='[m]')
        t_s = datetime.date.fromordinal(np.int(time[0, 0]))
        t_e = datetime.date.fromordinal(np.int(time[0, -1]))
        ax.set_title(self.ID + '  ' + self.project + '  ' + np.str(
            t_s.month) + '/' + np.str(t_s.day) + ' - ' + np.str(t_e.month) + '/' + np.str(t_e.day), fontsize=11)
    ...
    # Single Transect
    def transect_cross_section_0(self, bin_depth, sig0, ct, sa, lon, lat, dac_u, dac_v, profile_tags, sigth_levels, include_ends):
        deep_shr_max = 0.1
        deep_shr_max_dep = 3500
        ...
        return ds, dist, avg_ct_pd, avg_sa_pd, avg_sig_pd, v_g, vbt, \
               isopycdep, isopycx, mwe_lon, mwe_lat, DACe_MW, DACn_MW, profile_tags, shear, v_g_east, v_g_north
